...
export default function AppHeader({ studioName }: Props) {
  return (
    <header className="h-16 bg-white border-b border-line px-8 flex items-center gap-4 shrink-0 sticky top-0 z-10">
      {/* Search */}
      <div className="flex items-center gap-2 bg-panel border border-line rounded-xl px-3 h-10 w-72">
      ...
      {/* Studio badge */}
      <div className="flex items-center gap-2 bg-lime-soft border border-lime-border rounded-full px-3.5 py-1.5">
        <span className="w-[7px] h-[7px] rounded-full bg-lime animate-pulseDot" />
        <span className="text-[11.5px] font-bold text-lime-text">{studioName || 'Your studio'}</span>
      </div>
