        ...
        <div className="flex flex-col">
          {sources.map((s) => (
            <div key={s.id} className="flex items-center gap-3 px-5 py-3.5 border-b border-line-soft last:border-0 flex-wrap">
              <span className={`text-sm font-semibold flex-1 min-w-[140px] ${s.is_active ? 'text-ink-strong' : 'text-ink-muted line-through'}`}>
                {s.name}
              </span>

              <form action={toggleLeadSource.bind(null, s.id, !s.is_active)}>
                <button type="submit" className="btn-secondary text-xs h-8 px-3">
                  {s.is_active ? 'Turn off' : 'Turn on'}
                </button>
              </form>

              <form action={deleteLeadSource.bind(null, s.id)}>
                <button type="submit" className="btn-danger text-xs h-8 px-3">Remove</button>
              </form>
            </div>
          ))}
